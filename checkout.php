...
<?php
	...
	if(isset($_POST['checkOut'])){
		...
		if($order_user_type==2 || $flag==true){
			...
			foreach($_SESSION['cart'] as $id => $value){
				if(!in_array($value['product_store'],$o_to_store)){
					$o_to_store[] = $value['product_store'];
				}
			}

			for($i=0; $i<count($o_to_store); $i++){

				$order_unique_id = getUniqueorderID();

				$sql_for_checkout = "INSERT INTO orders(`OrderUniqueId`, `OrderUserID`, `OrderGuestName`, `OrderAmount`, `OrderQuntity`, `OrderShipName`,`OrderShipAddress`,`OrderCity`, `OrderZip`, `OrderCountry`, `OrderPhone`, `OrderMobile`, `OrderShippingCost`, `OrderEmail`, `OrderDate`,`OrderShipped`,`o_store`) 
												 VALUES('$order_unique_id','$order_user_id','$order_gust_neme','$order_amount','$order_quntity','$order_short_address','$order_detail_address','$order_disrict','$order_zip','$order_country','$order_phon','$order_mobile','$order_shipping_cost','$order_user_email', '$order_date','0','$o_to_store[$i]')";
